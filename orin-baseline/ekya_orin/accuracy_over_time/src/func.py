...
def inference(student_model, window, inference_config, inference_debug):   
    before_student_model, student_model = student_model
    window_time, retrain_time = inference_debug
    ...
    student_model.eval()
    corrects = 0
    unit_time = window_time / len(data_loader)
    elapsed_time = 0
    ...
    with torch.cuda.amp.autocast():
        for inputs, labels in tqdm.tqdm(data_loader, total=len(data_loader)):           
            ...
            with torch.no_grad():
                if elapsed_time < retrain_time:
                    outputs = before_student_model(inputs)
                else:
