...
from emulator.ekya_profiler import EkyaProfiler
...
class Emulator:
    def __init__(self, config: Config):
        ...
        if self.config.cl_type != "Ekya":
            raise ValueError(f"continual learning type must be STATIC, not {self.config.cl_type}")
            
        self.profiler: EkyaProfiler = EkyaProfiler(config=self.config)
        ...
        self.previous_labeled_indices: List[List[int]] = [None,]
        ...
        self.decision_log_path = f"{self.config.output_root}/profile_decision_log.csv"
        with open(self.decision_log_path, "w") as f:
            ...
            header = [
                ...
                "Sampling rate",
                "Epoch",
                "Num of retraining imgs",
                "Num of previous sampled imgs"
            ]
            csv_writer.writerow(header)
        # <<<<< log window dist <<<<<
    ...
    def __run(self,
        ...
        """
        TODO: Ekya-like profiling
        - Result sampling rate and epochs
        - Consider profiling time? -> currently no
        """
        profile_result, train_indices = self.profiler.profile(module=copy.deepcopy(self.student_model.module).cpu(),
                                                              entire_dataset=self.entire_dataset,
                                                              indices_to_micro_profile=self.previous_labeled_indices[window_idx])
        ...
        with open(self.decision_log_path, "a") as f:
            ...
            csv_writer.writerow([
                window_idx,
                profile_result.desired_sampling_rate,
                profile_result.epochs,
                len(train_indices),
                len(self.previous_labeled_indices[window_idx]) if window_idx > 0 else 0
            ])
        # <<<<< log decision <<<<<
        ...
        # >>>>> sub-sample by profiled result >>>>>
        # train_dataset = self.train_dataset_from_prev_window[window_idx]
        train_dataset = TrainSampleDataset(ori_dataset=self.entire_dataset,
    ...
    def __sample_data_to_train(self,
        ...
        self.previous_labeled_indices.append(copy.deepcopy(train_sample_indices))
