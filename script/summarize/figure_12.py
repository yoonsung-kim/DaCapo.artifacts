...
parser = argparse.ArgumentParser(description="summarize results for figure. 12")
...
def prepare_dc(name_to_save: str, dict, dst_root: Path):
    ...
    with open(dc_st["ori-correct"]) as f:
        ...
        # corrects = corrects.reshape(5, 12, 300).tolist()

        corrects = corrects.reshape(9, 12, 300).tolist()
...
if __name__ == "__main__":
    ...
    fig12_dir = summary_root / "figure_12"
    tmp_dir = fig12_dir / "tmp"
    ...
    model = "resnet18"
    s = "es1"
    es1_files = copy.deepcopy(template)

    cl_type = "spatiotemporal"
    es1_files["dc-st"]["ori-correct"] = \
        output_root / cl_type / model / "output" / s / "result.json"
    es1_files["dc-st"]["ori-end_point"] = \
        output_root / cl_type / model / "output" / s / "phase_log.csv"
    prepare_dc(name_to_save=f"{model}-{s}", dict=es1_files, dst_root=tmp_dir)
    
    cl_type = "eomu"
    es1_files["eomu"]["ori-correct"] = \
        output_root / cl_type / model / "output" / s / "result.json"
    es1_files["eomu"]["ori-end_point"] = \
        output_root / cl_type / model / "output" / s / "train_log.csv"
    prepare_eomu(name_to_save=f"{model}-{s}", dict=es1_files, dst_root=tmp_dir)
    
    cl_type = "orin_high-ekya"
    es1_files["ekya"]["correct"] = \
        output_root / "orin_high-ekya" / model / "output" / s / "result.json"
    es1_files["ekya"]["end_point"] = \
        output_root / "orin_high-ekya" / model / "output" / s / "result.json"
    
    if not os.path.isfile(es1_files["ekya"]["correct"]) or \
        not os.path.isfile(es1_files["ekya"]["end_point"]):
        es1_files["ekya"]["correct"] = ""
        es1_files["ekya"]["end_point"] = ""

    s = "es2"
    es2_files = copy.deepcopy(template)

    cl_type = "spatiotemporal"
    es2_files["dc-st"]["ori-correct"] = \
        output_root / cl_type / model / "output" / s / "result.json"
    es2_files["dc-st"]["ori-end_point"] = \
        output_root / cl_type / model / "output" / s / "phase_log.csv"
    prepare_dc(name_to_save=f"{model}-{s}", dict=es2_files, dst_root=tmp_dir)
    
    cl_type = "eomu"
    es2_files["eomu"]["ori-correct"] = \
        output_root / cl_type / model / "output" / s / "result.json"
    es2_files["eomu"]["ori-end_point"] = \
        output_root / cl_type / model / "output" / s / "train_log.csv"
    prepare_eomu(name_to_save=f"{model}-{s}", dict=es2_files, dst_root=tmp_dir)
    
    cl_type = "orin_high-ekya"
    es2_files["ekya"]["correct"] = \
        output_root / "orin_high-ekya" / model / "output" / s / "result.json"
    es2_files["ekya"]["end_point"] = \
        output_root / "orin_high-ekya" / model / "output" / s / "result.json"
    
    if not os.path.isfile(es2_files["ekya"]["correct"]) or \
        not os.path.isfile(es2_files["ekya"]["end_point"]):
        es2_files["ekya"]["correct"] = ""
        es2_files["ekya"]["end_point"] = ""
    # <<<<< file init <<<<<
    #TODO: start here
    ...
    def draw(drift_time_list, important_drift_time_list):
        ...
        plt.savefig(str(fig12_dir / save_file), dpi=dpi, format="pdf", bbox_inches="tight")
        plt.show()
    ...
    x_ticks, y_ticks = 120, 20
    start_y, end_y = 40, 100
    width, height, dpi = 600, 150, 100
    fontsize = 15
    ...
    # mystique_file = [f"./extreme/dacapo/extreme_{extreme_number}/corrects_per_phase.json",
    #                  f"./extreme/dacapo/extreme_{extreme_number}/retraining_end_points.json"]
    # mm_file = [f"./extreme/eomu/extreme_{extreme_number}/corrects.json",
    #            f"./extreme/eomu/extreme_{extreme_number}/retraining_end_points.json"]

    # mystique_file = ["", ""]
    # mm_file = ["", ""]
    # orin_file = [f"student_file-extreme_{extreme_number}-resnet18-wide_resnet50_2.json",
    #             f"student_file-extreme_{extreme_number}-resnet18-wide_resnet50_2.json"]

    # draw with end points
    mystique_file = [
        es1_files["dc-st"]["correct"],
        es1_files["dc-st"]["end_point"]
    ]
    mm_file = [
        es1_files["eomu"]["correct"],
        es1_files["eomu"]["end_point"]
    ]
    orin_file = [
        es1_files["ekya"]["correct"],
        es1_files["ekya"]["end_point"]
    ...
    draw([], [window_time * 1 + 60, window_time * 2 + 60, window_time * 3 + 60])
    ...
    # orin_file = [f"student_file-extreme_{extreme_number}-resnet18-wide_resnet50_2.json",
    #             f"student_file-extreme_{extreme_number}-resnet18-wide_resnet50_2.json"]
    # save_file = f"accuracy-over-time-extreme_{extreme_number}-resnet18-overall.pdf"

    # draw with end points
    mystique_file = [
        es2_files["dc-st"]["correct"],
        es2_files["dc-st"]["end_point"]
    ]
    mm_file = [
        es2_files["eomu"]["correct"],
        es2_files["eomu"]["end_point"]
    ]
    orin_file = [
        es2_files["ekya"]["correct"],
        es2_files["ekya"]["end_point"]
    ...
    # make table
    def gmean_dc_and_eomu(json_file):
        start = 4 * NUM_IMGS_PER_WINDOW
        num = 4 * NUM_IMGS_PER_WINDOW

        with open(json_file) as f:
            corrects = np.array(json.load(f)).flatten()
            corrects = corrects[start:start+num]
    ...
    def gmean_ekya(json_file):
        ...
        warmup_cnt = 4
        gmean_cnt = 4
        window = raw_data[window_frame * warmup_cnt: window_frame * (warmup_cnt + gmean_cnt)]

        avg_set = []
        for i in range(gmean_cnt):
            avg = sum(window[window_frame * i : window_frame * (i+1)]) / window_frame
            avg_set.append(avg)

        return gmean(avg_set) * 100

    table_path = fig12_dir / "figure_12-sheet.csv"
    with open(table_path, "w") as f:
        ...
        row = ["DaCapo"]
        row.append(f"{gmean_dc_and_eomu(es1_files['dc-st']['correct']):2.1f}%")
        row.append(f"{gmean_dc_and_eomu(es2_files['dc-st']['correct']):2.1f}%")
        csv_writer.writerow(row)

        row = ["EOMU"]
        row.append(f"{gmean_dc_and_eomu(es1_files['eomu']['correct']):2.1f}%")
        row.append(f"{gmean_dc_and_eomu(es2_files['eomu']['correct']):2.1f}%")
        csv_writer.writerow(row)

        row = ["Ekya"]
        row.append(f"{gmean_ekya(es1_files['ekya']['correct']):2.1f}%")
        row.append(f"{gmean_ekya(es2_files['ekya']['correct']):2.1f}%")
        csv_writer.writerow(row)



"""
def extream_ekya_gmean(file_name):
    import json
...
extream_ekya_gmean(f"./extreme/ekya/extreme_2/student_file.json")
"""